...
export default function LoginPage() {
  ...
  async function createSessionCookie(user: User) {
    // obține ID token de la Firebase și creează cookie de sesiune pe server
    const idToken = await user.getIdToken(true);
    const res = await fetch('/api/auth/login', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include', // asigură setarea cookie-ului în răspuns
      body: JSON.stringify({ idToken }),
    });
    if (!res.ok) {
      const txt = await res.text().catch(() => '');
      throw new Error('Nu s-a putut crea sesiunea. ' + txt);
    }
  }
  ...
  async function handleAuthSuccess(user: User) {
    ...
    try {
      ...
      console.error('Eroare la crearea sesiunii:', e?.message || e);
      toast({
        title: 'Eroare sesiune',
        description: 'Am autentificat contul, dar nu am putut crea sesiunea. Reîncearcă.',
        variant: 'destructive',
      });
      return;
    }
    // redirect stabil (fără să rămână login în istoric)
            ...
            <CardContent>
              ...
              <div className="mt-6">
                ...
                <Button variant="outline" className="w-full mt-4" onClick={handleGoogleSignIn} disabled={isLoading}>
                  <svg role="img" viewBox="0 0 24 24" className="mr-2 h-4 w-4" aria-label="Google">

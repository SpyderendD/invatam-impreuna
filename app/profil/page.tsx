...
function CustomTooltip({ active, payload, label }: any) {
  ...
  return <div className="rounded-lg border bg-background p-2 shadow-sm"><p className="font-bold text-foreground">{label}</p><p className="text-sm text-primary">Progres: {payload[0].value}%</p></div>;
}
...
export default function ProfilePage() {
  ...
  const stats = useMemo(() => {
    if (!progress) {
      return { lessonsCompleted: 0, avgScore: 0, level: "Începător" as const, perfectTests: 0, chartData: [], radarData: [], recentTests: [] };
    }
  ...
  const handleExport = () => { /* ... funcția ta de export ... */ };
            ...
            <AnimatePresence mode="wait">
              <motion.div key={activeTab} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.2 }}>
                      ...
                      return [
                        <motion.div key="stat-lessons" variants={itemVariants}>
                          <StatCard icon={<BookCheck className="h-5 w-5" />} title="Lecții finalizate" description="Tot ce ai marcat ca „complet”.">
                            <StatRing value={Math.min(100, (stats.lessonsCompleted / 50) * 100)}><AnimatedNumber value={stats.lessonsCompleted} /></StatRing>
                ...
                <TabsContent value="progress" className="mt-8 grid grid-cols-1 xl:grid-cols-2 gap-6">
                  <Card><CardHeader><CardTitle>Progres pe Materii</CardTitle><CardDescription>Procentul de lecții finalizate la fiecare materie.</CardDescription></CardHeader><CardContent>{stats.chartData.length > 0 ? (<ResponsiveContainer width="100%" height={300}><BarChart data={stats.chartData} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}><defs><linearGradient id="colorUv" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="hsl(var(--primary))" stopOpacity={0.85} /><stop offset="95%" stopColor="hsl(var(--primary))" stopOpacity={0.15} /></linearGradient></defs><CartesianGrid strokeDasharray="3 3" opacity={0.2} vertical={false} /><XAxis dataKey="name" stroke="hsl(var(--muted-foreground))" fontSize={12} tickLine={false} axisLine={false} /><YAxis stroke="hsl(var(--muted-foreground))" fontSize={12} tickLine={false} axisLine={false} /><RechartsTooltip cursor={{ fill: 'hsl(var(--accent))' }} content={<CustomTooltip />} /><Bar dataKey="progres" fill="url(#colorUv)" radius={[6, 6, 0, 0]} /></BarChart></ResponsiveContainer>) : (<p className="text-sm text-muted-foreground text-center py-10">Nu există date încă.</p>)}</CardContent></Card>
                  <Card><CardHeader><CardTitle>Puncte forte (Radar)</CardTitle><CardDescription>Unde stai mai bine, ca procent de lecții finalizate.</CardDescription></CardHeader><CardContent>{stats.radarData.length > 0 ? (<ResponsiveContainer width="100%" height={300}><RadarChart data={stats.radarData}><PolarGrid stroke="hsl(var(--border))" /><PolarAngleAxis dataKey="subject" stroke="hsl(var(--muted-foreground))" /><PolarRadiusAxis stroke="hsl(var(--muted-foreground))" /><Radar name="Progres" dataKey="value" stroke="hsl(var(--primary))" fill="hsl(var(--primary))" fillOpacity={0.25} /></RadarChart></ResponsiveContainer>) : (<p className="text-sm text-muted-foreground text-center py-10">Nu există date încă.</p>)}</CardContent></Card>

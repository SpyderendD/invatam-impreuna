...
import { ThemeToggle } from '@/components/theme/ThemeToggle'; // Importă ThemeToggle
...
export function Navbar() {
  ...
  return (
    <>
      {/* Header - Acum folosește bg-card (care e negru în dark mode) și border-border */}
      <header className="sticky top-0 w-full z-40 bg-card border-b border-border">
        <nav className="container h-16 flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center gap-6">
            {navLinks.map(link => (
              // text-foreground și hover:text-primary se adaptează temei
              <Link key={link.href} href={link.href} className="text-foreground text-sm font-medium hover:text-primary transition-colors">{link.label}</Link>
            ))}
          </div>

          <div className="flex items-center gap-4">
            <div className="hidden lg:flex items-center gap-4">
              <ThemeToggle /> {/* Plasează ThemeToggle aici */}
              {user ? (
                <DropdownMenu>
                  ...
                  {/* Culorile dropdown-ului se adaptează automat (bg-card, text-foreground) */}
                ...
                <>
                  {/* Buton Autentificare - text-primary se va adapta pentru contrast în Dark Mode */}
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setIsOpen(false)} className="fixed inset-0 bg-black/40 z-[99] lg:hidden">
            {/* Fundalul meniului lateral - bg-card/70 */}
            <motion.div variants={sidebarVariants} initial="hidden" animate="visible" exit="hidden" onClick={(e) => e.stopPropagation()} className="fixed top-0 right-0 bottom-0 w-5/6 max-w-sm bg-card/70 backdrop-blur-xl z-[100] shadow-2xl flex flex-col">
              ...
              <motion.div className="flex flex-col flex-1 px-4" variants={contentVariants} initial="hidden" animate="visible">
                ...
                <motion.nav variants={itemVariants} className="flex flex-col gap-2">{navLinks.map(link => (<Link key={link.href} href={link.href} onClick={() => setIsOpen(false)} className="flex items-center gap-4 text-lg font-medium text-foreground p-4 rounded-md hover:bg-card/50 transition-colors"><link.icon className="h-6 w-6 text-primary" /><span>{link.label}</span></Link>))}</motion.nav>
                <motion.hr variants={itemVariants} className="my-4 border-border" />

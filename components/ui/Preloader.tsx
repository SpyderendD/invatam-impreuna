...
export default function Preloader({ onFinish }: { onFinish?: () => void }) {
  ...
  useEffect(() => {
    // Logica pentru Countdown (cronometru zile)
    // Simulăm logica originală: scade zilele rapid
    const totalDuration = 2000; // Durata loaderului în ms (2 secunde)
    const intervalTime = totalDuration / 7;

    const timer = setInterval(() => {
      setDays((prev) => {
        if (prev <= 1) {
          clearInterval(timer);
          return 0; // Deadline atins
        }
        return prev - 1;
      });
    }, intervalTime);

    // Ascundem loaderul după ce termină
    const timeout = setTimeout(() => {
      setIsVisible(false);
      if (onFinish) onFinish();
    }, totalDuration + 500); // Puțin delay după ce ajunge la 0
  ...
  return (
    <AnimatePresence>
      {isVisible && (
        ...
        >
          {/* STILURILE CSS ORIGINALE (Adaptate) */}
          <style jsx>{`
            #deadline {
              width: 100%;
              max-width: 580px;
            ...
            @keyframes write {
              ...
              16% { transform: translate(0px, 0px) rotate(5deg) scale(0.8, 1); }
              32% { transform: translate(0px, 0px) rotate(0deg) scale(1, 1); }
              48% { transform: translate(0px, 0px) rotate(6deg) scale(0.8, 1); }
              65% { transform: translate(0px, 0px) rotate(0deg) scale(1, 1); }
              83% { transform: translate(0px, 0px) rotate(4deg) scale(0.8, 1); }
            }
            /* FLAMES */
            #red-flame { animation: show-flames 2s ease infinite, red-flame 120ms ease infinite; transform-origin: center bottom; }
            #yellow-flame { animation: show-flames 2s ease infinite, yellow-flame 120ms ease infinite; transform-origin: center bottom; opacity: 0.7; }
            #white-flame { animation: show-flames 2s ease infinite, red-flame 100ms ease infinite; transform-origin: center bottom; opacity: 0.8; }

            @keyframes show-flames { 0%, 100% { opacity: 0; } 80%, 99% { opacity: 1; } }
            @keyframes red-flame { 0%, 100% { transform: translateY(-30px) scale(1); } 50% { transform: translateY(-30px) scale(1.1); } }
            @keyframes yellow-flame { 0%, 100% { transform: translateY(-30px) scale(0.8); } 50% { transform: translateY(-30px) scale(1.1); } }
          `}</style>

          <div id="deadline">
            <svg preserveAspectRatio="xMidYMid meet" id="line" viewBox="0 0 581 158">
              <g id="fire">
                <rect id="mask-fire-black" x="511" y="41" width="38" height="34" fill="black"/>
                <g>
                  <defs>
                  ...
                  <g id="group-fire" clipPath="url(#mask-fire_1_)">
                    ...
                    <path id="yellow-flame" fill="#F7B523" d="M528.837,100.291c4.197,0,5.108-1.854,5.974-5.417 c0.902-3.724-1.129-6.207-5.305-9.931c-2.396-2.137-1.581-4.176-0.565-6.32c-4.401,1.918-3.384,5.304-2.482,6.658 c1.511,2.267,2.099,2.364,0.42,5.8c-1.679,3.435-5.42,0.764-4.275-1.527c-1.921,1.512-2.373,4.04-1.528,6.563 C522.057,99.051,525.994,100.291,528.837,100.291z"/>
                    <path id="white-flame" fill="#FFFFFF" d="M529.461,100.291c-2.364,0-4.174-1.322-4.129-3.469 c0.04-2.145,1.117-3.56,3.141-4.198c2.022-0.638,1.463-3.195,0.302-3.925c2.798,0.821,2.89,2.382,2.711,3.332 c-0.301,1.597-2.883,1.779-1.938,3.834c0.912,1.975,3.286,0.938,2.409-0.913c1.086,0.903,1.826,2.701,0.864,3.924 C532.18,99.691,531.064,100.291,529.461,100.291z"/>
                  </g>
                </g>
              </g>
            ...
            <div className="deadline-days">
              Se încarcă <span className="day-number">{days}</span> resurse...
            </div>
          </div>
        </motion.div>
      )}
